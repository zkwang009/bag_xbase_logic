...
class OR(AnalogBase):
    ...
    def _draw_layout_helper(self, lch, wn, wp, nfn, nfp, nf_inv, g_space, ds_space, ptap_w, ntap_w,
        ...
        inv_g = self.connect_to_tracks([inv_gn_warr, inv_gp_warr], inv_g_tid, min_len_mode=0)
        # connect gate
        o_b = self.connect_to_tracks([nor_n0_ports['d'], nor_n1_ports['d'], nor_p1_ports['d'], inv_g],
                                     nout_id, min_len_mode=0)
        # connect drain
