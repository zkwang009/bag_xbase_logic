...
class Cload(AnalogBase):
    ...
    def _draw_layout_helper(self, lch, wn, wp, nfn, nfp, g_space, ds_space, ptap_w, ntap_w,
                            g_width_ntr, ds_width_ntr, intent, ndum_side, show_pins,
                            power_width_ntr,
                            **kwargs):
        ...
        inv_dn_warr = self.connect_to_tracks(inv_n_ports['d'], nout_id, min_len_mode=0)
        inv_dp_warr = self.connect_to_tracks(inv_p_ports['d'], pout_id, min_len_mode=0)
        ...
        idx = self.grid.coord_to_nearest_track(inv_dn_warr.layer_id + 1, inv_dn_warr.upper)
